...
// ── Stat cards — 6 cards (Needs Review removed, Rejected is merged) ───────────
...
const CARDS = [
  ...
  {
    // Key is 'rejected' — backend merges REJECTED + FINANCE_REJECTED + NEEDS_REVIEW
    key: 'rejected',
    label: 'Rejected',
    sub: 'All rejections',
    icon: (
    ...
    // status=REJECTED — backend getMyRequests expands this to all three rejection statuses
    onClick: (navigate) => navigate('/requests?status=REJECTED'),
  },
  {
    key: 'cancelled',
    label: 'Cancelled',
    sub: 'Aborted requests',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      ...
      <div className="max-w-[1200px] mx-auto space-y-5 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <Link to="/requests/new"
            className="self-start sm:self-auto flex items-center gap-2 bg-amber-500
                       hover:bg-amber-600 text-white font-bold px-5 py-2.5 rounded-xl
                       text-sm shadow-lg shadow-amber-500/20 transition-all hover:scale-105">
        ...
        <div className={card + ' overflow-hidden'}>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4
            ...
            <div className="flex items-center gap-2">
              ...
              <Link to="/requests"
                className="text-xs font-bold text-amber-500 hover:text-amber-600 whitespace-nowrap">
                View all →
              </Link>
            </div>
          </div>
                ...
                {!filtered.length ? (
                  <tr>
                    <td colSpan={4} className="text-center py-12">
                      ...
                      {!search && (
                        <Link to="/requests/new"
                          className="inline-block mt-2 text-xs text-amber-500 font-bold hover:underline">
                          Create your first →
                        </Link>
                      )}
                    </td>
                  ...
                  <tr key={req.id}
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        {/* Status dot — amber = needs action, red = permanently closed */}
